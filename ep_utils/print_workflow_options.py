...
def print_available_workflows_and_tools(paramsD, print_workflows=True, print_rules=False, only_rules=False):
    ...
    if print_workflows:
        ...
        sys.stdout.write(pretty_name("Available Eelpond Workflows") +'\n')
        ep_flows = paramsD.get('eelpond_workflows')
    ...
    if print_rules:
        ...
        rules_dir = paramsD['eelpond_directories'].get('rules', 'rules')
...
if __name__ == '__main__':
    ...
    args = p.parse_args()
    # read params here, to enable passing in paramsD instead of paramsfile from run_eelpond
